...
import { DEFAULT_DAILY_LIMITS, type ProviderId, API_ERROR_CODES } from '@clipforge/shared';
...
/**
 * Throw 429 if user is at quota for a hard-capped model. Soft-capped models
 * log a warning but allow the generation.
 */
export async function enforceQuota(userId: number, provider: ProviderId) {
  ...
  if (remaining <= 0) {
    if (isHardCap) {
      throw new ApiError(429, API_ERROR_CODES.QUOTA_EXCEEDED, 'Daily quota exceeded', {
        provider,
        limit,
        resetAt: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(),
      });
    }
    // Soft cap — log and allow
  }
}